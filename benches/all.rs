...
use hloo::{Index, MemMapIndex, MemoryIndex};
...
fn index_search_comparison(c: &mut Criterion) {
    ...
    let mut index1 = MemoryIndex::new(Permuter::get(3));
    ...
    group.bench_function("in-memory", |b| b.iter(|| index1.search(target, 5)));
    group.bench_function("mem-mapped", |b| b.iter(|| index2.search(target, 5)));
...
fn search_comparison(c: &mut Criterion) {
    ...
    println!("inserting data into in-memory...");
    let mut lookup1 = Permuter::create_in_memory_lookup::<i64>();
    lookup1.insert(&data).unwrap();
    println!("inserting data into mem-mapped...");
    let mut lookup2 = Permuter::create_mem_map_lookup::<i64>(temp_dir.path()).unwrap();
    lookup2.insert(&data).unwrap();

    let mut group = c.benchmark_group("search 10_000_000");

    group.bench_function("naive", |b| {
        b.iter(|| hloo::index::scan_block(&data, &target, 5, |x1, x2| x1.xor_count_ones(x2)))
    });
    group.bench_function("hloo in-memory", |b| b.iter(|| lookup1.search(target, 5)));
    group.bench_function("hloo mem-mapped", |b| b.iter(|| lookup2.search(target, 5)));
...
fn insert_comparison(c: &mut Criterion) {
    ...
    group.bench_function("mem-mapped", |b| {
        b.iter(|| {
            let temp_dir = tempfile::tempdir().unwrap();
            let mut lookup = Permuter::create_mem_map_lookup::<i64>(temp_dir.path()).unwrap();
            lookup.insert(&data).unwrap();
        })
    });
